...
type (
	...
	Access interface {
		...
		Exists(ctx context.Context, element *models.UploadElement) bool

		// GetReader returns a reader for the specified in the element
		// returns (reader for the file|error)
		GetReader(ctx context.Context, element *models.UploadElement) (io.ReadSeekCloser, error)
		...
		GetWriter(ctx context.Context, element *models.UploadElement) (io.WriteCloser, error)
	}
	...
	Repo interface {
		...
		// source archive
		ExtractManifestFiles(ctx context.Context, element *models.UploadElement) error

		// List all versions of a certain package existing
		// - `scope` of the package
		// - `name` of the package
		// returns (releases found|nil if not exists, error)
		List(ctx context.Context, scope string, name string) ([]models.ListElement, error)
		...
		EncodeBase64(ctx context.Context, element *models.UploadElement) (string, error)

		// PublishDate returns the date element was upload / published
		PublishDate(ctx context.Context, element *models.UploadElement) (time.Time, error)

		// Checksum provides the sha256 checksum of the element
		// returns (checksum string|empty string if not exists, error)
		Checksum(ctx context.Context, element *models.UploadElement) (string, error)
		...
		LoadMetadata(ctx context.Context, scope string, name string, version string) (map[string]any, error)
		...
		GetAlternativeManifests(ctx context.Context, element *models.UploadElement) ([]models.UploadElement, error)
		...
		GetSwiftToolVersion(ctx context.Context, manifest *models.UploadElement) (string, error)

		// Lookup returns the list of identifiers for the provided url
		Lookup(ctx context.Context, url string) []string

		// Remove deletes the provided element
		// - `element` to be removed
		// returns nil if successful otherwise error
		Remove(ctx context.Context, element *models.UploadElement) error
		...
		ListInScope(ctx context.Context, scope string) ([]models.ListElement, error)

		// ListAll returns all packages across all scopes
		// returns (list of all packages|error)
		ListAll(ctx context.Context) ([]models.ListElement, error)
		...
		LoadPackageJson(ctx context.Context, scope string, name string, version string) (map[string]any, error)
	}
)
