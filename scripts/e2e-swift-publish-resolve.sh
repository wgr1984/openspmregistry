...
for i in $(seq 1 30); do
	if curl -s --connect-timeout 2 "${REGISTRY_URL}/" >/dev/null 2>&1; then
		break
	fi
	if [ "$i" -eq 30 ]; then
...
swift package-registry publish "$PACKAGE_ID" "$VERSION" --url "$REGISTRY_URL" --allow-insecure-http

echo "Verifying package metadata..."
ACCEPT_JSON="Accept: application/vnd.swift.registry.v1+json"
INFO_JSON=$(curl -sf -H "$ACCEPT_JSON" "${REGISTRY_URL}/${SCOPE}/${PACKAGE_NAME}/${VERSION}")
...
MANIFEST_58=$(curl -sf -H "$ACCEPT_SWIFT" "${REGISTRY_URL}/${SCOPE}/${PACKAGE_NAME}/${VERSION}/Package.swift?swift-version=5.8")
