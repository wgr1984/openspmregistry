#!/usr/bin/env bash
# Delete example/SamplePackage components from Nexus private repo.
...
BASE_URL="$NEXUS_URL/service/rest/v1/search?repository=$REPO&group=example&name=SamplePackage"
DELETED=0
TOKEN=""
...
if [ "$DELETED" -gt 0 ]; then
	echo "Cleaned $DELETED example.SamplePackage component(s) from Nexus"
fi
